...
class ForwardFashion(nn.Cell):
    def __init__(self, num_class=10):  # 一共分十类，图片通道数是1
        ...
        # 卷积层(input, filter - 卷积核，stride - 步长，padding - 补洞策略)
        ...
        # 线性整流 / 斜坡函数
        # 进入神经元的来自上一层神经网络的输入向量，使用线性整流激活函数的神经元会输出至下一层神经元或作为整个神经网络的输出
        self.relu = nn.ReLU()
        # 华为开源mindspore框架
        # Flattern() - 返回一维数组，用于作为全连接层的输入
...
class ForwardFashionRegularization(nn.Cell):
    def __init__(self, num_class=10):  # 一共分十类，图片通道数是1
        ...
        self.maxpool2d = nn.MaxPool2d(kernel_size=2, stride=2)
        self.relu = nn.ReLU()
        # Dropout层 - 用于减少神经网络拟合，根据相应概率去除部分神经元
        self.dropout = nn.Dropout()
        self.flatten = nn.Flatten()
        self.fc1 = nn.Dense(3200, 128)
        # 对神经网络的每一层进行归一化，假设将每一层输出后的数据都归一化到0均值，1方差，满足正态分布
